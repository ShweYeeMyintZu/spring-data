...
""", Double.class);
    ...
    public double avgModernWay(){
        return jdbcTemplate.query("select * from employee",this::mapEmployee).stream().map(Employee::getSalary)
                .mapToDouble(s->s).average().orElse(0);
    }

    public void createEmployee(Employee emp){
        jdbcTemplate
                .update("insert into employee(id,first_name,last_name,email,salary) values(?,?,?,?,?)",
                emp.getId(),emp.getFirstName(),emp.getLastName(),emp.getEmail(),emp.getSalary());
    }
    ...
    public double avgResultSetExtractor() {
        ...
        return jdbcTemplate.query("""
                select salary from employee""", resultExtractor);
    }
