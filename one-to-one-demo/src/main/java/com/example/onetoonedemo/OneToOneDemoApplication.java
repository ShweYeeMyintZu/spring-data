...
public class OneToOneDemoApplication implements CommandLineRunner {
 ...
 customerService.createDb();
        ...
        System.out.println(String.format("%s lives in %s.",customer.getName(),customer.getAddress().getCity()));

        util.JPAUtil.checkData("select * from customer");
        util.JPAUtil.checkData("select * from address");
